...
hpBar_border = pg.Rect(400,900,200,25)#border rect is made outside the loop because it doesn't change. hpBar's rect is made inside the loop because HEALTHPOINTS changes and hpBar's width has to change correspondingly
...
while running:
    ...
    if game_over:
        ...
        game_over = False
        HEALTHPOINTS = 100
        ants = pg.sprite.Group()#essentially a list to control our ant objects of the ant class
    ...
    #draw hp bar
    hpBar = pg.Rect(400,900,HEALTHPOINTS*2,25) #HEALTHPOINTS multiplied by 2 because the original width is 200 and our HEALTHPOINTS becomes our width and HEALTHPOINTS max is 100.  100*2=200. To not have to *2, make HEALTHPOINTS max as 200.
    pg.draw.rect(screen,pg.Color('green'),hpBar)
    pg.draw.rect(screen, pg.Color('white'),hpBar_border,1)
    ...
    #draw HIGHSCORE 
    score_surface = font.render("Cookie High Score: " + str(int(HIGHSCORE)), True, pg.Color('white'))
    score_rect = score_surface.get_rect()
    score_rect.center = (150,950)
